...
func (r *availabilityRepo) ListByUser(ctx context.Context, userID string) ([]entity.AvailabilitySlot, error) {
	rows, err := r.db.QueryContext(ctx,
		`SELECT id, user_id, day_of_week, start_time, end_time, slot_type, is_active, created_at
		 FROM availability_slots WHERE user_id = $1 ORDER BY day_of_week, start_time`, userID,
	)
	if err != nil {
		return nil, fmt.Errorf("list availability slots: %w", err)
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		if err := rows.Scan(
			&s.ID, &s.UserID, &s.DayOfWeek, &s.StartTime, &s.EndTime,
			&s.SlotType, &s.IsActive, &s.CreatedAt,
		); err != nil {
...
func (r *availabilityRepo) Create(ctx context.Context, slot *entity.AvailabilitySlot) error {
	...
	if err != nil {
		return fmt.Errorf("create availability slot: %w", err)
	}
	return nil
}
...
func (r *availabilityRepo) Delete(ctx context.Context, id, userID string) error {
	...
	if err != nil {
		return fmt.Errorf("delete availability slot: %w", err)
	}
	return nil
}
