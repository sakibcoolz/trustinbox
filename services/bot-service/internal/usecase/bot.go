...
func (uc *BotUseCase) CreateBot(ctx context.Context, spID, name, purpose, department, industryProfileID, createdByUserID, avatarURL string) (*entity.Bot, error) {
	...
	evt := events.NewEvent(events.BotCreated, "bot-service", map[string]string{
		"bot_id":     bot.ID,
		"bot_name":   bot.Name,
	...
	if err := uc.publisher.Publish(ctx, evt); err != nil {
		uc.log.Error("failed to publish bot created event", zap.Error(err))
	}
...
func (uc *BotUseCase) UpdateBot(ctx context.Context, botID, spID, name, purpose, department, avatarURL, status string) (*entity.Bot, error) {
	...
	// Publish activation event when bot transitions to active
	if prevStatus != string(entity.BotStatusActive) && bot.Status == entity.BotStatusActive {
		evt := events.NewEvent(events.BotActivated, "bot-service", map[string]string{
			"bot_id":   bot.ID,
			"bot_name": bot.Name,
		}).WithOrg(spID)
		if err := uc.publisher.Publish(ctx, evt); err != nil {
			uc.log.Error("failed to publish bot activated event", zap.Error(err))
		}
	}
...
func (uc *BotUseCase) DeleteBot(ctx context.Context, botID, spID string) error {
	...
	evt := events.NewEvent(events.BotArchived, "bot-service", map[string]string{
		"bot_id":   bot.ID,
		"bot_name": bot.Name,
	}).WithOrg(spID)
	if err := uc.publisher.Publish(ctx, evt); err != nil {
		uc.log.Error("failed to publish bot archived event", zap.Error(err))
	}
	return nil
}
...
func (uc *BotUseCase) ExecuteAction(ctx context.Context, botID, spID, conversationID, userID, actionType, toolName, inputJSON string) (string, bool, error) {
	...
	if !policyAllowed {
		...
		if err := uc.actionRepo.Create(ctx, actionLog); err != nil {
			uc.log.Error("failed to log denied bot action", zap.Error(err))
		}
	...
	if err := uc.actionRepo.Create(ctx, actionLog); err != nil {
		uc.log.Error("failed to log bot action", zap.Error(err))
	}

	// Update analytics
	if err := uc.statsRepo.IncrementActions(ctx, botID); err != nil {
		uc.log.Error("failed to increment bot action analytics", zap.Error(err))
	}

	// Publish action event
	evt := events.NewEvent(events.BotActionExecuted, "bot-service", map[string]string{
		"bot_id":          botID,
		"tool_name":       toolName,
		"conversation_id": conversationID,
		"policy_decision": "ALLOW",
	}).WithUser(userID).WithOrg(spID)
	if err := uc.publisher.Publish(ctx, evt); err != nil {
		uc.log.Error("failed to publish bot action executed event", zap.Error(err))
	}
