...
func main() {
	...
	// Repository
	profileRepo := postgres.NewIndustryProfileRepository(db)

	// Use case
	profileUC := usecase.NewIndustryProfileUseCase(profileRepo)

	// gRPC handler
	handler := grpcdelivery.NewIndustryProfileHandler(profileUC)
	...
	log.Info("shutting down industry service")
	srv.GracefulStop()
}
