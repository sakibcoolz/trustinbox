...
- [x] Open Provider Portal → navigate to `/conversations`
- [x] Click "New Conversation" or "Start Conversation" button
- [x] Select a customer (virtual ID)
- [x] Verify `useCreateConversation()` fires POST `/api/conversations`
  - Payload: `{ userId: "<customer-uuid>", serviceProviderId: "<sp-id>" }`
- [x] Verify `communication-service.CreateConversation()` executes:
  - Creates conversation entity with status `OPEN`
  - Persists to PostgreSQL
  - Publishes `conversation.created` event
- [x] Verify redirect to `/conversations/<new-id>`
- [x] Verify conversation appears in conversation list with:
...
- [x] In the conversation detail page, type a message:
  - Message: "Hello, how can we assist you today?"
- [x] Click send (or press Enter)
- [x] Verify `useSendMessage()` fires POST `/api/conversations/{id}/messages`
  - Payload: `{ content: "Hello, how can we assist you today?", messageType: "TEXT" }`
- [x] Verify `communication-service.SendMessage()`:
  - Persists message to PostgreSQL
  - Status: `SENT`
  - Publishes `message.sent` event
- [x] Verify message appears in provider's message thread:
...
- [x] Open Web App → navigate to `/conversations`
- [x] Verify new conversation appears in conversation list:
  - SP name displayed
  - Last message preview: "Hello, how can we assist you today?"
  - Unread indicator
- [x] Click on conversation to open chat area
- [x] Verify message displayed:
  - Content: "Hello, how can we assist you today?"
  - Sender type: AGENT (blue bubble)
  - Timestamp
- [x] Verify XMPP connection:
...
- [x] In Web App chat area, type a reply:
  - Message: "I have a question about my account"
- [x] Click send
- [x] Verify XMPP stanza sent:
  - Message type: TEXT
  - To: `org-<conv-id>@conference.chat.trustinbox.local`
- [x] Verify `communication-service` persists the message:
  - Sender type: CUSTOMER
  - Status: SENT → DELIVERED
- [x] Verify message appears in Web App chat:
  - Customer bubble (right-aligned in web app)
  - Delivery receipt indicator

### Sub-task 6.3.5 — Provider Portal: Receive Reply

- [x] Switch to Provider Portal → conversation detail page
- [x] Verify customer's reply appears in message thread:
...
- [x] Verify read receipt on provider's original message updates:
  - ✓ SENT → ✓✓ DELIVERED (when web app received)
  - ✓✓ READ (when customer opened)

### Sub-task 6.3.6 — File Attachment Transfer

- [x] In Provider Portal conversation, attach a file:
...
- [x] Switch to Web App conversation:
...
- [x] Provider: create conversation → appears in list with OPEN status
- [x] Provider: send message → appears in thread with SENT receipt
- [x] Web App: conversation received via XMPP → message displayed correctly
- [x] Web App: customer replies → message sent via XMPP stanza
- [x] Provider: customer reply appears in real-time via SSE
- [x] File attachment: provider uploads → web app previews → downloadable → integrity ok
- [x] File attachment: reverse direction (customer → provider) also works
- [x] Typing indicators: bidirectional with proper timeout behavior
- [x] All messages persist in PostgreSQL and load correctly after page refresh
- [x] Read receipts: SENT → DELIVERED → READ status progression visible
- [x] Conversation history: correct ordering, sender types, timestamps across refreshes
- [x] Jaeger: traces for create conversation, send message, and XMPP message delivery
