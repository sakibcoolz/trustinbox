...
export default function FriendsPage() {
  const { token } = useAuth();
  const { onFriendEvent } = useNotifications();
  ...
  async function handleAccept(reqId: string) {
    ...
    try {
      ...
      if (res.ok) {
        ...
        const data = await res.json();
        toastError('Failed', data.error || 'Could not accept request');
      }
    } catch {
  ...
  async function handleReject(reqId: string) {
    ...
    try {
      ...
      if (res.ok) {
        ...
        const data = await res.json();
        toastError('Failed', data.error || 'Could not decline request');
      }
    } catch {
      toastError('Network error');
    } finally {
  ...
  async function handleSendRequest(userId: string) {
    ...
    try {
      ...
      if (res.ok) {
        ...
        toastWarning(data.error || 'Could not send request');
      }
    } catch {
      toastError('Network error');
    } finally {
            ...
            <>
              ...
              {filteredFriends.length === 0 && filteredOutgoing.length === 0 && (
                <EmptyState
                  ...
                  description="Connect with people to start messaging securely."
          ...
          {tab === 'find' && (
            <div className="px-4 space-y-4">
              ...
              {findSearch.length < 2 && (
                <EmptyState
                  icon={Search}
                  title="Search for people to connect"
