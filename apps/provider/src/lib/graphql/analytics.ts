...
export function useDailyAnalytics(vars: AnalyticsDateVars) {
  const result = useData<{ entries: DailyAnalyticsEntry[] }>(analyticsUrl('daily', vars), { skip: !vars.serviceProviderId, deps: [vars.from, vars.to] });
  return { ...result, data: result.data ? { dailyAnalytics: result.data.entries ?? [] } : undefined };
}
